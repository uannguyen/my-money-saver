...
Bật chế độ bảo mật số tiền, đặt PIN 6 số để khoá thông tin nhạy cảm (lương, số dư, tiết kiệm). Số tiền được hiển thị dạng `••••` khi chưa mở khoá.

### Cài đặt

Quản lý tài khoản, bảo mật PIN, tuỳ chỉnh danh mục, cấu hình giao dịch định kỳ và xuất dữ liệu Excel.
...
- **Quỹ tiết kiệm** — tạo quỹ (tiền mặt, ngân hàng, khẩn cấp, đầu tư…), nạp/rút tiền, xem lịch sử biến động số dư, theo dõi ngày đáo hạn
- **Bảo mật số tiền (PIN)** — ẩn lương, tổng thu, số dư và tiết kiệm sau khi mở app; PIN 6 số hash PBKDF2-SHA256 phía client
...
```
src/
├── components/        # UI components (budget, transactions, charts, goals, privacy, common, layout)
├── contexts/          # AuthContext, PrivacyContext (PIN lock + amount masking)
...
├── services/          # Firestore CRUD (transactionService, budgetService, recurringService, privacyService, ...)
